...
also the best distance ever recorded in that race. To guarantee you win the grand prize, you need to make sure
...
For each whole millisecond you spend at the beginning of the
race holding down the button, the boat's speed increases by one millimeter per millisecond.
...
def extract_race_data():
    return list(zip(re.findall(r'(\d+)', data[0]), re.findall(r'(\d+)', data[1])))


def get_combined_race_data():
    race_data = extract_race_data()
    return ''.join(race[0] for race in race_data), ''.join(race[1] for race in race_data)


def get_number_of_winning_times(record):
...
def part1():
    total = 1
    for record in extract_race_data():
        total *= get_number_of_winning_times(record)

    print(total)


def part2():
    print(get_number_of_winning_times(get_combined_race_data()))
