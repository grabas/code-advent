...
In the above example, card 1 has five winning numbers (41, 48, 83, 86, and 17) and eight numbersyou have
...
resource = "day_04.txt"
scratchcards = get_input_file_contents(resource)


def split_scratchcard(scratchcard):
    winning_numbers, scratched_numbers = re.sub(r'Card\s+\d+:', '', scratchcard).strip().split('|')

    winning_numbers = [int(num) for num in winning_numbers.split()]
    scratched_numbers = [int(num) for num in scratched_numbers.split()]

    return set(winning_numbers), set(scratched_numbers)


def get_winning_numbers(scratchcard):
    winning_numbers, scratched_numbers = split_scratchcard(scratchcard)

    return list((winning_numbers.intersection(scratched_numbers)))


def get_points(scratchcard):
    winning_numbers = get_winning_numbers(scratchcard)
    return math.floor(2 ** (len(winning_numbers) - 1))


def process_card(card, index, total_cards):
    number_of_winning_numbers = len(get_winning_numbers(card)) + 1
    total_cards += 1

    for i in range(1, number_of_winning_numbers):
        next_card_index = index + i
        if next_card_index < len(scratchcards):
            next_card = scratchcards[next_card_index]
            total_cards = process_card(next_card, next_card_index, total_cards)
...
def part2():
    ...
    for index, card in enumerate(scratchcards):
        total_cards = process_card(card, index, total_cards)
